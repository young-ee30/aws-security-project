...
**왜 이게 중요한가**: GitHub Actions 워크플로우에서 이 이름들을 그대로 사용합니다. 예를 들어 `build-push-ecr.yml`에는 이런 코드가 있습니다:
...
bootstrap/ 실행 (로컬, 1회)
    ...
    → 버킷 이름을 GitHub Secret에 등록: TF_STATE_BUCKET
...
**GitHub Actions에서 backend.hcl 생성 방법 (`terraform.yml` 워크플로우)**:
```yaml
...
`TF_STATE_BUCKET` Secret에 버킷 이름이 저장되어 있고, 워크플로우 실행 시마다 `backend.hcl` 파일을 동적으로 생성해서 사용합니다.
...
GitHub Actions `build-push-ecr.yml`이 트리거되어 이미지를 빌드하고 ECS에 배포합니다.
...
**② IAM Role 생성: ECR/ECS 배포용**

- 이름: `GitHubActions-ECR-Role`
- 권한 정책: `AmazonEC2ContainerRegistryPowerUser`
- 추가 권한: `AmazonECS_FullAccess` (ECS 배포용)
...
**③ IAM Role 생성: Terraform용**

- 이름: `GitHubActions-Terraform-Role`
...
|---|---|---|
| `AWS_ROLE_ARN` | `arn:aws:iam::282146511585:role/GitHubActions-ECR-Role` | build-push-ecr.yml |
| `AWS_ROLE_ARN_TERRAFORM` | `arn:aws:iam::282146511585:role/GitHubActions-Terraform-Role` | terraform.yml |
| `TF_STATE_BUCKET` | S3 버킷 이름 (아래 참고) | terraform.yml |

> `TF_STATE_BUCKET` 값 확인 방법:
> `terraform/bootstrap/terraform.tfstate` 파일을 열면 `bucket_name` 출력값이 있습니다.
...
| `TF_STATE_BUCKET` 관련 에러 | Secret 미등록 | 9번 과정 확인 |
