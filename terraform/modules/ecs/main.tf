...
resource "aws_ecs_service" "service" {
  ...
  # GitHub Actions 배포 워크플로우가 최신 이미지를 반영하더라도,
  # Terraform이 서비스 스케일 의도(active_backend 기준 desired_count)를 유지하도록
  # task_definition만 무시하고 desired_count는 Terraform이 계속 관리합니다.
  lifecycle {
