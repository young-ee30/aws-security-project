...
DEFAULT_STATE_PROJECT_NAME="${TF_STATE_PROJECT_NAME:-devsecops}"
STATE_BUCKET_PREFIX="${TF_STATE_BUCKET_PREFIX:-${DEFAULT_STATE_PROJECT_NAME}-tfstate-}"
...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--delete-state-bucket]

Options:
  --delete-state-bucket  Also delete the bootstrap Terraform state bucket created by GitHub Actions.
...
resolve_deterministic_state_bucket() {
  local aws_region="$1"
  local account_id=""
  local target_bucket=""

  if [ -n "${TF_STATE_BUCKET:-}" ] && aws s3api head-bucket --bucket "${TF_STATE_BUCKET}" >/dev/null 2>&1; then
  ...
  target_bucket="${DEFAULT_STATE_PROJECT_NAME}-tfstate-${account_id}-${aws_region}"
...
resolve_state_bucket() {
  ...
  deterministic_bucket="$(resolve_deterministic_state_bucket "${aws_region}")"
  if [ -n "${deterministic_bucket}" ] && [ "${deterministic_bucket}" != "None" ]; then
    echo "${deterministic_bucket}"
    return 0
  fi
...
TOTAL_STEPS=3
if [ "${DELETE_STATE_BUCKET}" -eq 1 ]; then
  TOTAL_STEPS=4
fi
...
echo ""
echo "[0/${TOTAL_STEPS}] Resolving Terraform state bucket..."
STATE_BUCKET="$(resolve_state_bucket "${AWS_REGION}")"
...
if [ -z "${STATE_BUCKET}" ] || [ "${STATE_BUCKET}" = "None" ]; then
  ...
  write_backend_hcl "${STATE_BUCKET}" "${AWS_REGION}"
...
if [ "${USE_TERRAFORM_DESTROY}" -eq 1 ]; then
  ...
  echo ""
  echo "[1/${TOTAL_STEPS}] terraform init..."
...
if [ "${USE_TERRAFORM_DESTROY}" -eq 1 ]; then
  echo ""
  echo "[2/${TOTAL_STEPS}] terraform destroy... (may take several minutes)"
  echo ""
...
echo ""
echo "[3/${TOTAL_STEPS}] best-effort AWS cleanup..."
best_effort_cleanup "${AWS_REGION}" "${NAME_PREFIX}"

if [ "${DELETE_STATE_BUCKET}" -eq 1 ]; then
  echo ""
  echo "[4/${TOTAL_STEPS}] deleting bootstrap Terraform state bucket..."

  if [ -n "${STATE_BUCKET}" ] && [ "${STATE_BUCKET}" != "None" ]; then
    empty_and_delete_bucket "${STATE_BUCKET}"
  else
    warn "No Terraform state bucket was resolved, so bootstrap bucket deletion was skipped."
  fi
fi
...
echo ""
if [ "${DELETE_STATE_BUCKET}" -eq 1 ]; then
  echo "  Bootstrap S3 state bucket deletion was requested."
  echo "  GitHub Actions bootstrap will recreate it on the next Terraform run if needed."
else
  echo "  Note: The bootstrap S3 state bucket is preserved."
  echo "  Re-run with --delete-state-bucket to remove it too."
fi
if [ "${TERRAFORM_DESTROY_EXIT}" -ne 0 ]; then
